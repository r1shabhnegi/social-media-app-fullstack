...
const signIn =
  ...
  async (req: Request, res: Response) => {
    ...
    if (parsedInput.error) {
      ...
      // ApiError("Invalid Credentials", AUTH_INVALID_CREDENTIALS, 401);
    }
    ...
    if (!foundUser) {
      ...
      //  ApiError("User Not Found", AUTH_USER_NOT_FOUND, 404);
    }
    ...
    if (!isMatch) {
      throw new Error("PW");
      // ApiError("pw", AUTH_INVALID_PW, 401);
    }
...
const refreshToken =
  ...
  async (req: Request, res: Response) => {
    ...
    if (!cookie?.jwt) {
      ...
      //  ApiError("Cookie Missing", RF_COOKIE_MISSING, 403);
    }
    ...
    if (!foundUser) {
      ...
      //  ApiError("Bad user request", RF_INVALID_USER, 401);
    }
    ...
    if (!decodedToken || foundUser._id.toString() !== decodedToken.userId) {
      ...
      // ApiError("Invalid Refresh Token", RF_INVALID_RF_TOKEN, 401);
    }
